...
CFLAGS_ARM64 := -m64
CHIBI_CFLAGS := $(filter-out -fvisibility=hidden -Os, $(CFLAGS)) -g -O0
TARGETS += makecapk/lib/arm64-v8a/lib$(APPNAME).so

$(CHIBI_TARGET_ARM64):
	mkdir -p makecapk/lib/arm64-v8a
	$(MAKE) -C $(CHIBI_SCHEME_DIR) android \
		CC='$(CC_ARM64)' \
...
$(CHIBI_ASSETS_DIR): $(CHIBI_SCHEME_DIR)/lib
	@echo "Copying Chibi Scheme .scm and .sld files to Sources/assets/."
	mkdir -p $@
	cd $(CHIBI_SCHEME_DIR)/lib && find . \( -name "*.scm" -o -name "*.sld" \) \
		-exec cp --parents {} ../../$@/ \;
	@echo "Chibi Scheme assets copied"
